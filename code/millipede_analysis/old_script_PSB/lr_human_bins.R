...
#     browser()
  ...
  if ((cv == T) | (weights != "LR")){
    ...
    if ( weights != "Equal"){
      ...
      image_data(DNaseData[rank_fold,], pwm[rank_fold], cbind(labels.df[rank_fold,]), rank_all, label_name, zMax_DNase = 7, fileName, pngWidth = 3)
...
#       rank_subset = c(sample(n0,min(1000, length(n0))), sample(n1,length(n1)))
      ...
      image_bins(DNaseData[rank_fold,], results.df[,1:7], cbind(labels.df[rank_fold,]), rank_all, modelNames[1:7], label_name, zMax_DNase = 7, fileName, pngWidth = 5)
      
      results = cbind(results.df$PostPr_Shrinkage, results.df$M5)
      model_name = c("CENTIPEDE", "M5")
      fileName = paste(dir_results, "fold", k, "_weights", weights, "_heatmap_cpd", sep = "")
      image_bins(DNaseData[rank_fold,], results, cbind(labels.df[rank_fold,]), rank_all, model_name, label_name, zMax_DNase = 7, fileName, pngWidth = 3)
    }
