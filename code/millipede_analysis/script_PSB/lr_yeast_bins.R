...
#   browser()
  if ((cv == T) | (weights != "LR")){
    ...
    if ( weights != "Equal"){
      ...
      image_data(DNaseData[rank_fold,], pwm[rank_fold], cbind(labels.df[rank_fold,]), rank_all, label_name, zMax_DNase = 20, fileName, pngWidth = 3)
      ...
      image_bins(DNaseData[rank_fold,], results.df[,1:6], cbind(labels.df[rank_fold,]), rank_subset, modelNames[1:6], label_name, zMax_DNase = 20, fileName, pngWidth = 5)
      
      results = cbind(results.df$PostPr_NoShrinkage, results.df$M5)
      model_name = c("CENTIPEDE", "M5")
      fileName = paste(dir_results, simpleCap(tf), "_fold", k, "_weights", weights, "_heatmap_cpd", sep = "")
      image_bins(DNaseData[rank_fold,], results, cbind(labels.df[rank_fold,]), rank_subset, model_name, label_name, zMax_DNase = 20, fileName, pngWidth = 3)
    }
